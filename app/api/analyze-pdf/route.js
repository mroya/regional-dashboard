...
IMPORTANTE:
- Na parte "geral", preencha diasUteis, diasRestantes, performanceGeral (Performance Acumulada do Mes). Procure a linha "Total :" da tabela REGIONAL e extraia "Tkt Méd" (tktMed) e "%Ev Tkt" (evTkt). Procure a linha "Total :" da tabela MEDICAMENTO TOTAL e extraia "%Desv" (medDesv) e "%Evl Vda" (medEvlVda). Procure a linha "Total :" da tabela GENÉRICO e extraia "%Desv" (genDesv) e "%Evl Vda" (genEvlVda). Procure a linha "Total :" da tabela HB (Não Medicamento) e extraia "%Desv" (hbDesv) e "%Evl Vda" (hbEvlVda).
...
FORMATO JSON:
{
  "geral": { "diasUteis": "31", "diasRestantes": "24", "performanceGeral": "...", "tktMed": "...", "evTkt": "...", "medDesv": "...", "medEvlVda": "...", "genDesv": "...", "genEvlVda": "...", "hbDesv": "...", "hbEvlVda": "..." },
...
export async function POST(request) {
  try {
    ...
    // Cache Inteligente
    // Adicionado "v7" para invalidar o cache antigo e forçar a extração de HB
    const textHash = crypto.createHash('sha256').update(limitedText + "v7").digest('hex');
